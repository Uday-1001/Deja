...
public:
    ...
    ll byspaceop_more(vector<int>&nums , int col)
    {
        int n = nums.size();
        vector<ll>curr_row(col+1 , 0);
        ...
        for(int i = n-1 ; i >= 0 ; i--)
        {
            for(int flag = 0 ; flag < 2 ; flag++)
            {
                ...
                if(flag == true)
                {
                    ll even_sum = nums[i] + curr_row[1-flag];
                    include = max<ll>(include , even_sum);
                }
                else
                {
                    ll odd_sum = -nums[i] + curr_row[1-flag];
                    include = max<ll>(include , odd_sum);
                }
                ll exclude = 0 + curr_row[flag];

            curr_row[flag] = max<ll>(include , exclude);
            }
        }
    return curr_row[1];  
    }

    // ll byspaceop(vector<int>&nums , int col)
    // {
    //     int n = nums.size();
    //     vector<ll>curr_row(col+1 , 0);
    //     vector<ll>next_row(col+1 , 0);
    ...
    //             if(flag == true)
    //             {
    //                 ll even_sum = nums[i] + next_row[1-flag];
    //                 include = max<ll>(include , even_sum);
    //             }
    //             else
    //             {
    //                 ll odd_sum = -nums[i] + next_row[1-flag];
    //                 include = max<ll>(include , odd_sum);
    //             }
    //             ll exclude = 0 + next_row[flag];
    ...
    long long maxAlternatingSum(vector<int>& nums) {    
        ...
        // return byspaceop(nums , 2);

        return byspaceop_more(nums , 2);
    }
};