...
public:
    vector<vector<int>> verticalTraversal(TreeNode* root) {
        ...
        while(!q.empty())
        {
            ...
            //fetch the coordinates from the given pair
            int row_cord = coordinates.first;
            int col_cord = coordinates.second;
            
            //stores the col and the row values in the multiset in the map
        ...
        for(auto &it: mp)
        {
            ...
            //storing a single line of the multiset map from mp
            vector<int>line;
            for(auto &col_map_it : col_map)
            {
                auto multi_set = col_map_it.second;

                //line.end() denotes the position of the iterator when using the insert operation in vector.
                //as the inserted value is of multiset , push_back operation cant be used.
