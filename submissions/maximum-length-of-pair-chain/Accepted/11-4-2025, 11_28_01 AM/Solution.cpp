...
public:
    ...
    int byspaceop_more(vector<vector<int>>&pairs)
    {
        int n = pairs.size();
        vector<int>curr_col(n+1 , 0);
        ...
        for(int curr = n-1 ; curr >= 0 ; curr--)
        {
            for(int prev = curr-1 ; prev >= -1 ; prev--)
            {
                ...
                {
                    include = 1 + curr_col[curr+1];
                }
                int exclude = 0 + curr_col[prev+1];

                curr_col[prev+1] = max(include , exclude);
            }
        }
    return curr_col[0];
    }

    // int byspaceop(vector<vector<int>>&pairs)
    // {
    //     int n = pairs.size();
    //     vector<int>curr_col(n+1 , 0);
    //     vector<int>next_col(n+1 , 0);
    ...
    //             if(prev == -1 or pairs[prev][1] < pairs[curr][0])
    //             {
    //                 include = 1 + next_col[curr+1];
    //             }
    //             int exclude = 0 + next_col[prev+1];
    ...
    int findLongestChain(vector<vector<int>>& pairs) {
        ...
        // return byspaceop(pairs);

        return byspaceop_more(pairs);
