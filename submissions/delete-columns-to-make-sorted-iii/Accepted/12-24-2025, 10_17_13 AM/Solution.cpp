...
public:
    ...
    int byrec(vector<string>&strs , int prev , int curr , vector<vector<int>>&dp)
    {
        //base case 
        ...
        {
            dont_remove = 0 + byrec(strs , curr , curr + 1 , dp);
            ans = min(ans , dont_remove);
        }
        int remove = 1 + byrec(strs , prev , curr + 1 , dp);
        ans = min(ans , remove);
    ...
    int minDeletionSize(vector<string>& strs) {
        ...
        return byrec(strs , -1 , 0 , dp) == INT_MAX ? 0 : byrec(strs , -1 , 0 , dp);
    }
};