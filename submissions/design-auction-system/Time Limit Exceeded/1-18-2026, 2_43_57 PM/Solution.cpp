...
public:
    ...
    void addBid(int userId, int itemId, long long bidAmount) {
        mp[itemId][userId] = bidAmount;
    }
    
    void updateBid(int userId, int itemId, long long newAmount) {
    ...
    int getHighestBidder(int itemId) {
        if(!mp.count(itemId)) return -1;

        long long biggestamt = -1;
        int biggestuser = -1;
